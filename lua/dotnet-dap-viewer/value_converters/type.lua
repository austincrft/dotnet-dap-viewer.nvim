...
return {
  ...
  extract = function(_, vars, var_path, _, cb)
    require("dotnet-dap-viewer.value_converters").vars_to_table(var_path, vars, function(val) cb(val, string.format("{ Name = %s, FullName = %s}", val.Name.value, val.FullName.value)) end)
  end,
}
