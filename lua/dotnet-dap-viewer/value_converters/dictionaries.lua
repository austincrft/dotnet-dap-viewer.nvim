...
return {
  ...
  extract = function(_, vars, var_path, _, cb)
    ...
    local netcoredbg = require("dotnet-dap-viewer.dap")
    ...
    netcoredbg.fetch_variables(var_ref, 1, function(entries)
      ...
      cb(
        result,
        is_complex_keys and require("dotnet-dap-viewer.pretty_printers.kv-pair-list").pretty_print(result) or require("dotnet-dap-viewer.pretty_printers.catch-all").pretty_print(result)
      )
    end)
  end,
}
